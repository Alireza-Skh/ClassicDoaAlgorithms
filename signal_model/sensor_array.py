...
class ULA:
    ...
    def p_signal(self, snr_db:float, shape=None) -> np.ndarray:
        if shape is None:
        ...
        if self.baseband_mode:
            ...
            doppler_freqs = self.rng.uniform(-self.freq * 0.1, self.freq*0.1, self.num_targets)
            random_phases = self.rng.uniform(0, 2*np.pi, self.num_targets)
        ...
        if self.coherent:
            ...
            sig = np.tile(reference_signal, (self.num_targets, 1))

        random_amplitudes = self.rng.uniform(0.1, 1.0, (self.num_targets, 1))
