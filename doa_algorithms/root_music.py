...
class RootMUSIC:
    ...
    def estimate(self, input_signal: np.ndarray):
        ...
        if self.array.num_targets >= self.array.num_antenna:
            raise ValueError(f"Number of sources ({self.array.num_targets}) must be less than number of antennas ({self.array.num_antenna})")

        noise_eigenvectors = eigenvectors[:, self.array.num_targets:]
        ...
        if len(inside_roots) < self.array.num_targets:
            roots_sorted = roots[np.argsort(np.abs(np.abs(roots) - 1.0))]
            selected_roots = roots_sorted[:self.array.num_targets]
        else:
            inside_roots_sorted = inside_roots[np.argsort(np.abs(inside_roots))[::-1]]
            selected_roots = inside_roots_sorted[:self.array.num_targets]
